...
[Önceki yazımızda](https://bb-tr-kaynak.netlify.app/algorithms/analysis-of-algorithms/asymptotic-analysis) asimptotik analizin algoritma analizinde problemlerle nasıl başa çıktığını tartışmıştık. Şimdi şu konulara göz atalım:

- Asimptotik analizin genel görünümü
...
Bir algoritmanın zaman karmaşıklığının ortalama durumudur (average case). Bir fonksiyonda Big-O notasyonunun ve Omega notasyonunun ortalama durumudur.
...
Ortalama durum analizi pratik ölçümler yapmak için çok kullanışlı değildir. Nadiren kullanılır. Bu analizde bütün olası girişlerin matematiksel dağılımını bilmeliyiz.

### 3-En İyi Durum Analizi

En iyi durum analizinde bir algoritmada herhangi bir alt sınır (lower bound) garanti etmek bize bir bilgi sağlamaz. Bir algoritmanın çalışması yıllar alabilir.
...
<details>
  ...
  <div>
    ...
    <div>
      Diğer sıralama algoritmalarının çoğu en kötü ve en iyi durumlara
      sahiptirler.
    </div>
    <br />
    <details>
      <summary>Örnek</summary>
      <div>
        Tipik bir Quik Sort uygulamasında (köşe elemanı olarak pivot
        seçildiğinde) en kötü durum; giriş dizisi önceden sıralandığında meydana
        gelir. En iyi durum ise pivot elementler her zaman diziyi iki yarıya
        böler.
      </div>
    </details>
    <br />
...
### 2-Aşağıdaki Örnekte Bir Dizinin Uzunluğunu Alacağız ve Şu Durumlarla İlgileneceğiz:

- Eğer (n) çift ise çıktısı 0’dır.
- Eğer (n) tek ise dizideki elemanları toplayacağız

Yukarıda saydığımız iki madde için aşağıda farklı programlama dillerinde bir örnek yer almaktadır:
...
Büyüme eğrisi **sabit (constant)** olacaktır çünkü biz en iyi durumda (n)’in çift olduğunu varsayarız.
...
<Tabs>
  <TabItem value="youtube-videos" label="Youtube videoları" default>
    <ul>
      <li>
        <a href="https://youtu.be/3bhBo9YCTpo">
  ...
  <TabItem value="questions" label="Sorular">
    <ul>
      <li>
        <a href="https://www.geeksforgeeks.org/practice-questions-time-complexity-analysis/">
